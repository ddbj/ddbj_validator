...
##
## ソートされる前の　submit されたオリジナルの tsv を submission_form から取得していたので、ここで実際の DB に格納されているアルファベット順にわざわざソートしていた。
## 2016-02-09 の改修移行、このソート処理をやると DB (tsv 順)、cgi 出力 (ソートされた順) になって sample name が更新されてしまうので、処理を外した
##
...
if $submitter_id != ""
	...
	begin
		...
		q1 = "SELECT submission_id, sample_name, attribute_value FROM mass.attribute attr LEFT OUTER JOIN mass.sample USING(smp_id) LEFT OUTER JOIN mass.submission USING(submission_id) WHERE submitter_id = '#{$submitter_id}' AND attr.attribute_name = 'sample_title'"
...
# name のアカウント内でのユニークネスをチェックする関数
def uniq_check_name(attrs, name_a)
	...
	attrs.each{|item|
		...
		for name in name_a
			...
			# submission id が異なっており title が同じ
			if name[1] == item && $submission_id != name[0]
				
				if report != ""
					report += ", duplicated with #{name[0]}:#{name[1]}"
				else
					report += "duplicated with #{name[0]}:#{name[1]}"
				end
		...
		if attrs.uniq.size == 1
			...
			organism_a.size.times do
				if organism_a[i] == sciname_a[i]
					return_a.push("identical [#{organism_a[i]}]")
				else
...
def check_bp(attrs)
	...
	if psub_query_id != ""
		begin
			...
			q1 = "SELECT 'PRJDB' || p.project_id_counter prjd, x.submission_id, x.content, p.status_id, sub.submitter_id, p.project_type FROM mass.project p LEFT OUTER JOIN mass.xml x USING(submission_id) LEFT OUTER JOIN mass.submission sub USING(submission_id) WHERE x.submission_id IN #{psub_query_id} AND (x.submission_id, x.version) IN (SELECT submission_id, MAX(version) from xml GROUP BY submission_id) ORDER BY submission_id"
			
			res1 = conn.exec(q1)

			res1.each do |r1|
				if r1["prjd"]
					bp_xml_a.push([r1["prjd"], r1["submission_id"], r1["content"], r1["status_id"], r1["submitter_id"], r1["project_type"]])
				else
					bp_xml_a.push(["", r1["submission_id"], r1["content"], r1["status_id"], r1["submitter_id"], r1["project_type"]])
				end
			end
	...
	if prjd_query_id != ""
		begin
			...
			q1 = "SELECT 'PRJDB' || p.project_id_counter prjd, x.submission_id, x.content, p.status_id, sub.submitter_id, p.project_type  FROM mass.project p LEFT OUTER JOIN mass.xml x USING(submission_id) LEFT OUTER JOIN mass.submission sub USING(submission_id) WHERE p.project_id_counter IN #{prjd_query_id} AND (x.submission_id, x.version) IN (SELECT submission_id, MAX(version) from xml GROUP BY submission_id) ORDER BY submission_id"
 	...
 	# XML パースして情報を格納
	for p, s, x, st, submi, ptype in bp_xml_a
		...
		# project type - primary or umbrella
		pro.store("project_type", ptype)
	...
	attrs.each{|bp|
		...
		for project_h in uniq_project_a
			if bp == project_h["PRJD"]
				...
				locus_tag_return_a.push(project_h["LocusTagPrefix"])

				# project type - primary or umbrella
				if project_h["project_type"] == "primary"
					project_type_return_a.push("")
				else
					project_type_return_a.push("Umbrella is referenced")
				end
		...
		if !found
			...
			locus_tag_return_a.push("")
		end

	}

	# プロジェクト情報と PRJD 番号とタイトルを返す
	return project_return_a, prjd_return_a, project_title_return_a, locus_tag_return_a, project_type_return_a
...
def get_all_locus_tags()
	...
	begin
		...
		# DDBJ BioSample 属性値として記録されている全 locus tag prefix を取得
		q1 = "SELECT attribute_value FROM mass.attribute WHERE attribute_name = 'locus_tag_prefix' AND attribute_value <> ''"
		
		res1 = conn.exec(q1)

		res1.each do |r1|
			all_locus_tags_a.push(r1["attribute_value"])
		end
	...
	for item in attr_a
		...
		# 属性名ごとの処理
			...
			when "sample_name"
				...
				# ssub 内での重複チェック - submit できないので実施しない
				# アカウント内での重複チェック
				result_a = uniq_check_name(item[1..-1], sample_title_a)
			...
			when "bioproject_id"
				...
				project_a, prjd_a, project_title_a, locus_tag_a, project_type_a = check_bp(item[1..-1])

				unless project_a.empty? || project_a.all? {|e| e.empty? }
					checked_a.push(["##{item[0]}"] + project_a)
				end

				# project type が Umbrella ではない
				unless project_type_a.empty? || project_type_a.all? {|e| e.empty? }
					checked_a.push(["##{item[0]}"] + project_type_a)
				end
			...
			when "locus_tag_prefix"
				...
				# locus tag 一致チェック
				unless item[1..-1].nil? || item[1..-1].empty? || item[1..-1].all? {|e| e.empty? }

					i = 0
					for tag in item[1..-1]
						if tag == locus_tag_a[i]
							result_a.push("")
						else
							result_a.push("Not in BioProject. BioProject:#{locus_tag_a[i]}")
						end
						
						# tag の submission 内での重複チェック
						unless tag.nil? || tag.empty?
							if item[1..-1].count(tag) > 1

								if result_a.empty?
									result_a.push("#{tag} duplicated in this submission")
								else
									result_a[i] = "#{result_a[i]}, tag #{tag} duplicated in this submission"
								end

							end
						end

						# tag の DDBJ BioSample 全体でのユニークネスチェック
						all_locus_tags_a = get_all_locus_tags()

						unless tag.nil? || tag.empty?
							if all_locus_tags_a.include?(tag)

								if result_a.empty?
									result_a.push("#{tag} duplicated in DDBJ BioSample")
								else
									result_a[i] = "#{result_a[i]}, tag #{tag} duplicated in DDBJ BioSample"
								end
...
for line in checked_a
	...
	for item in replaced_attr_a
		if line[0] == item[0].sub("!", "")
			if line[0] == "*organism"
				line[1..-1] = item[1..-1].collect{|organism|
					organism.sub(/identical /, "").sub(/\[/, "").sub(/\]/, "")
				}				
			else
				line[1..-1] = item[1..-1]
			end
