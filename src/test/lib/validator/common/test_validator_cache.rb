...
class TestValidatorCache < Minitest::Test
  ...
  def test_chach_package_versus_organism
    ...
    ret1 = @validator.send("package_versus_organism", "48", "SampleA", taxonomy_id, package_name, organism_name, 1)
    cache = @validator.instance_variable_get (:@cache)
    #p cache.instance_variable_get (:@cache_data)
    ...
    ret2 = @validator.send("package_versus_organism", "48", "SampleA", taxonomy_id, package_name, organism_name, 1)
    assert_equal true, ret1 == ret2
    ...
    organism_name = "Homo sapiens"
    cache_key = ValidatorCache::create_key(taxonomy_id, package_name)
    ret3 = @validator.send("package_versus_organism", "48", "SampleA", taxonomy_id, package_name, organism_name, 1)
    cache = @validator.instance_variable_get (:@cache)
    #p cache.instance_variable_get (:@cache_data)
    assert_equal true, cache.has_key(ValidatorCache::TAX_VS_PACKAGE, cache_key)
    # expected output "use cache in package_versus_organism" when executes debug mode
    ret4 = @validator.send("package_versus_organism", "48", "SampleA", taxonomy_id, package_name, organism_name, 1)
  ...
  def test_cache_sex_for_bacteria
    ...
    ret1 = @validator.send("sex_for_bacteria", "59", "SampleA", taxonomy_id, sex, organism_name, 1)
    cache = @validator.instance_variable_get (:@cache)
    #p cache.instance_variable_get (:@cache_data)
    ...
    ret2 = @validator.send("sex_for_bacteria", "59", "SampleA", taxonomy_id, sex, organism_name, 1)
    assert_equal true, ret1 == ret2
    ...
    ret3 = @validator.send("sex_for_bacteria", "59", "SampleA", taxonomy_id, sex, organism_name, 1)
    cache = @validator.instance_variable_get (:@cache)
    #p cache.instance_variable_get (:@cache_data)
    assert_equal true, cache.has_key(ValidatorCache::TAX_HAS_LINAGE, cache_key)
    # expected output "use cache in sex_for_bacteria(fungi)" when executes debug mode
    ret4 = @validator.send("sex_for_bacteria", "59", "SampleA", taxonomy_id, sex, organism_name, 1)
    assert_equal true, ret3 == ret4
  end
end
