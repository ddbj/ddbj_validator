...
class TestXmlConvertor < Minitest::Test
  ...
  def test_text2obj
    ...
    biosample_list = @convertor.text2obj(xml_doc)
  ...
  def test_location_from_attrname
    xpath_sample_name = @convertor.location_from_attrname("sample_name" , 2)
    ...
    xpath_sample_title = @convertor.location_from_attrname("sample_title" , 2)
    assert_equal ["//BioSample[2]/Description/Title"], xpath_sample_title

    xpath_description = @convertor.location_from_attrname("description" , 2)
    assert_equal ["//BioSample[2]/Description/Comment/Paragraph"], xpath_description

    xpath_organism = @convertor.location_from_attrname("organism" , 2)
    assert_equal ["//BioSample[2]/Description/Organism/OrganismName"], xpath_organism

    xpath_taxonomy_id = @convertor.location_from_attrname("taxonomy_id" , 2)
    assert_equal ["//BioSample[2]/Description/Organism/@taxonomy_id"], xpath_taxonomy_id

    xpath_bioproject_accession = @convertor.location_from_attrname("bioproject_id" , 2)
  ...
  def test_location_of_attrname
    xpath_attr_name = @convertor.location_of_attrname("attr_name" , 2)
    assert_equal ["//BioSample[2]/Attributes/Attribute[@attribute_name=\"attr_name\"]/@attribute_name"], xpath_attr_name
  end
end
