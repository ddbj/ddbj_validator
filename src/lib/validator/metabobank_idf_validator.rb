...
class MetaboBankIdfValidator < ValidatorBase
  ...
  def initialize
    super()
    @conf.merge!(read_config(File.absolute_path(File.dirname(__FILE__) + "/../../conf/metabobank")))
    ...
    @json_schema = JSON.parse(File.read(File.absolute_path(File.dirname(__FILE__) + "/../../conf/metabobank/schema.json")))
